...
const queries = {
    ...
    countries: "select countryid, name, population from Countries",
    alldirectors: "select * from Directors where directorname not like '%Rep Etido%'",
    getAll: "SELECT  rec.[id],[title],[rating],[yearFilm],[ratedate],[ratehour],dir.directorname,[directorid],[country],[poster],[director2] FROM [Filmmemo].[dbo].[FilmRecord] rec join [Filmmemo].[dbo].[Directors] dir on rec.directorid = dir.id where rating is not null order by ratedate desc",
    getAllWithNotSeen: "SELECT  rec.[id],[title],[rating],[yearFilm],[ratedate],[ratehour],dir.directorname,[directorid],[country],[poster],[director2] FROM [Filmmemo].[dbo].[FilmRecord] rec join [Filmmemo].[dbo].[Directors] dir on rec.directorid = dir.id order by ratedate desc",
...
join [Filmmemo].[dbo].[Directors] dir on films.directorid = dir.id  where films.id =`,
    ...
    getTotalsCountry: "SELECT [country], name, COUNT(*) AS record_count, ROUND((COUNT(*) * 100.0 / SUM(COUNT(*)) OVER ()), 2) AS percentage, AVG(REC.rating * 1.0) AS average FROM [Filmmemo].[dbo].[FilmRecord] REC JOIN [Filmmemo].[dbo].[Countries] CTR on REC.country = CTR.countryid WHERE REC.rating is not null GROUP BY [country], name order by record_count desc",
